{
  ...
  outputs = { self, nixpkgs }:
    let
      ...
      pythonEnv = pkgs: pkgs.python3.withPackages (ps: [ ps.pygobject3 ps.pycairo ]);
      testInputs = pkgs: {
        nativeBuildInputs = [ pkgs.gobject-introspection pkgs.wrapGAppsHook4 ];
        buildInputs = [
          (pkgs.python3.withPackages (ps: [ ps.pygobject3 ps.pycairo ps.pytest ]))
          pkgs.gtk4 pkgs.xvfb-run pkgs.dbus
        ];
      };
    in
    {
      packages = forAllSystems (pkgs: {
        default = pkgs.stdenv.mkDerivation {
          ...
          buildInputs = [ (pythonEnv pkgs) pkgs.gtk4 ];
      ...
      checks = forAllSystems (pkgs: {
        e2e = pkgs.stdenv.mkDerivation ((testInputs pkgs) // {
          name = "waiverserv-e2e";
          src = ./.;
        ...
        modules = [
          ...
          let
            ...
            pickOutput = pkgs.writeText "waiverserv-pick-output.py" ''
              ...
              transform = "90" if best["width"] > best["height"] else "normal"
          ...
          in
          {
            ...
            systemd.services.waiverserv-provision = {
              ...
              before = [ "wpa_supplicant.service" "waiverserv-sync.service" "sshd.service" ];
